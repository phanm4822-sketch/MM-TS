"""Command-line options and the fixed MM-TS architecture."""
...
def build_parser(defaults: dict, description: str):
    ...
    parser.add_argument(
        ...
        help="directory for generated caches",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
    ...
    parser.add_argument(
        "--model_parallel",
        type=str2bool,
        default=defaults["model_parallel"],
        help="enable device_map=auto",
    )
    parser.add_argument(
        "--eval_only",
        type=str2bool,
        default=defaults["eval_only"],
    ...
    parser.add_argument(
        "--save_checkpoint",
        type=str2bool,
        default=defaults["save_checkpoint"],
        help="save best checkpoint",
    )
    parser.add_argument(
        "--rebuild_cache",
        type=str2bool,
        default=defaults["rebuild_cache"],
        help="force rebuild cache from csv",
    )
    parser.add_argument(
        "--cache_batch_size",
        type=int,
        default=defaults["cache_batch_size"],
    ...
    parser.add_argument(
        "--vision_render_size",
        type=int,
        default=defaults["vision_render_size"],
    ...
    parser.add_argument(
        "--max_train_batches",
        type=int,
        default=defaults["max_train_batches"],
        help="limit train batches per epoch",
    )
    parser.add_argument(
        "--max_eval_batches",
        type=int,
        default=defaults["max_eval_batches"],
        help="limit eval batches",
    )
    parser.add_argument(
        "--few_shot_ratio",
        type=float,
        default=defaults["few_shot_ratio"],
        help="fraction of the original train split to keep via seed-controlled random sampling; 1.0 keeps full training data",
    )
    parser.add_argument(
        "--eval_denorm",
        type=str2bool,
        default=defaults["eval_denorm"],
        help="report denormalized metrics",
    )
    parser.add_argument(
        "--eval_test_during_train",
        type=str2bool,
        default=defaults["eval_test_during_train"],
        help="evaluate test split every epoch",
    )
    parser.add_argument(
        "--print_per_var_metrics",
        type=str2bool,
        default=defaults["print_per_var_metrics"],
    ...
    parser.add_argument(
        "--img_scale",
        type=float,
        default=defaults["img_scale"],
        help="fixed multiplicative scale on image tokens",
    )
    parser.add_argument(
        "--vid_scale",
        type=float,
        default=defaults["vid_scale"],
        help="fixed multiplicative scale on video tokens",
    )
    parser.add_argument(
        "--text_scale",
        type=float,
        default=defaults["text_scale"],
        help="fixed multiplicative scale on text tokens",
    )
    parser.add_argument(
        "--ts_bias_scale",
        type=float,
        default=defaults["ts_bias_scale"],
        help="fixed TS bias scale when not learnable",
    )
    parser.add_argument(
        "--ts_bias_learnable",
        type=str2bool,
        default=defaults["ts_bias_learnable"],
        help="learnable TS bias scale",
    )
    parser.add_argument(
        "--use_time_features",
        type=str2bool,
        default=defaults["use_time_features"],
        help="add time/channel embeddings to TS tokens",
    )
    parser.add_argument(
        "--time_embed_scale",
        type=float,
        default=defaults["time_embed_scale"],
        help="time/channel embedding scale",
    )
    parser.add_argument(
        "--use_ts_attn_bias",
        type=str2bool,
        default=defaults["use_ts_attn_bias"],
        help="inject TS attention bias",
    )
    parser.add_argument(
        "--ts_attn_bias_layers",
        type=str,
        default=defaults["ts_attn_bias_layers"],
        help="comma-separated layer indices for TS bias, or 'all'",
    )
    parser.add_argument(
        "--ts_bias_dtw_weight",
        type=float,
        default=defaults["ts_bias_dtw_weight"],
        help="DTW bias weight",
    )
    parser.add_argument(
        "--ts_bias_cov_weight",
        type=float,
        default=defaults["ts_bias_cov_weight"],
        help="covariance bias weight",
    )
    parser.add_argument(
        "--ts_bias_pear_weight",
        type=float,
        default=defaults["ts_bias_pear_weight"],
        help="Pearson bias weight",
    )
    parser.add_argument(
        "--use_ts_residual",
        type=str2bool,
        default=defaults["use_ts_residual"],
    ...
    parser.add_argument(
        "--lora_target_layers",
        type=str,
        default=defaults["lora_target_layers"],
    ...
    parser.add_argument(
        "--prompt_max_tokens",
        type=int,
        default=defaults["prompt_max_tokens"],
    ...
    parser.add_argument(
        "--revin_eps", type=float, default=defaults["revin_eps"], help="RevIN epsilon"
    )
    parser.add_argument(
        "--revin_subtract_last",
        type=str2bool,
        default=defaults["revin_subtract_last"],
